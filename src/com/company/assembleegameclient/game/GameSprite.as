...
public function bannerSystemAdd():void {
      // Create BannerManager if it doesn't exist or recreate it with current map reference
      if (!bannerManager) {
         bannerManager = new BannerManager(this.map);
      } else {
...
/*   var bannerShape:Shape = BulkBannerSystem.getBanner(guildId, 16);
  ...
  */
   ...
   private function onBannerSystemClosed(e:Event):void {
      bannerSystemRemove();
   }

   public function bannerSystemRemove():void {
      if (bannerManager) {
         bannerManager.removeEventListener("bannerSystemClosed", onBannerSystemClosed);
         bannerManager.bannerSystemRemove();

         if (contains(bannerManager)) {
            removeChild(bannerManager);
         }
         // Don't set bannerManager to null - keep it for reuse
      }
